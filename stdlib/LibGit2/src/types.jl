...
@kwdef struct TransferProgress
    total_objects::Cuint    = 0
    indexed_objects::Cuint  = 0
    received_objects::Cuint = 0
    local_objects::Cuint    = 0
    total_deltas::Cuint     = 0
    indexed_deltas::Cuint   = 0
    received_bytes::Csize_t = 0
end
...
@kwdef struct DescribeOptions
    ...
    only_follow_first_parent::Cint    = 0
    show_commit_oid_as_fallback::Cint = 0
end
...
@kwdef struct PushOptionsStruct
    version::Cuint                     = 1
    parallelism::Cint                  = 1
    callbacks::RemoteCallbacksStruct   = RemoteCallbackStruct()
    @static if LibGit2.VERSION >= v"0.25.0"
        proxy_opts::ProxyOptions       = ProxyStruct()
    end
    @static if LibGit2.VERSION >= v"0.24.0"
        custom_headers::StrArrayStruct = StrArrayStruct()
    end
...
@kwdef struct BlameHunk
    lines_in_hunk::Csize_t                = 0

    final_commit_id::GitHash              = GitHash()
    final_start_line_number::Csize_t      = 0
    ...
    orig_start_line_number::Csize_t       = 0
